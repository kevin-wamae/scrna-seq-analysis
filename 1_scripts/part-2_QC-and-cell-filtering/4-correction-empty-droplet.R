...
#   EmptyDrops avoids threshold bias by testing the expression profile (gene
#   mix) instead of total count numbers. It:
...
#   - If Total Droplets Tested: ~1,000,000 to 1,500,000 barcodes.
...
#
# INTERPRETING YOUR OUTPUT, For example (Your output will vary):
#   - Droplets tested: 1,380,000 -> Normal partition capacity.
#   - Cells called: 9,600 (0.7%) -> Great yield for a standard single channel.
#   - Empty droplets: 1,389,600 (99.3%) -> Normal. Most captures contain soup.
...
#   Single-cell UMI ranges are too vast for standard linear graphing. A plot
#   with a cell at 50,000 UMIs squashes a cell with 100 UMIs flat against the
#   y-axis. Log10 conversion spreads out the scale by counting the number
#   of "zeros":
...
#     the algorithm successfully distinguished background soup from healthy
#     cells

# Create a dataframe of the single-cell data for plotting
empty_df <- data.frame(
...
#   soup that was drawn into every single droplet—including our healthy ones.
#
#   In Step 5, we will pass this filtered matrix to SoupX. We will use the
#   unfiltered count matrix we purposely cached right before filtering
#   (`RAW_COUNTS_ALL_DROPLETS`) to profile this background soup, calculate a
#   sample-specific contamination fraction, and mathematically wash ambient
#   transcripts directly out of our cells.
# ****************************************************************************#
