...
#   They are submerged in an extracellular liquid suspension saturated with
#   free-floating RNA molecules that leaked out of fragile cells that lysed
#   (burst) during tissue dissociation.
#
# WHY AMBIENT RNA MATTERS:
#   This background "soup" is pulled into EVERY droplet alongside your true
#   cells. It acts as a universal contaminant, creating false-positive gene
#   expression. It severely inflates marker genes and heavily skews tissue-
#   specific profiles (e.g., free-floating Hemoglobin flooding immune cells
#   in blood, or Myelin transcripts contaminating non-neuronal cells in brain
#   tissue). If left uncorrected, it misleads downstream clustering and
#   compromises the statistical specificity of your differential expression
#   analysis.
#
# HOW SOUPX RESOLVES IT:
#   SoupX calculates the exact expression fingerprint of the empty droplets,
#   profiles your cell-containing droplets, and uses advanced regression
#   modelling to figure out exactly how much ambient background noise needs to
#   be subtracted out of your true expression counts.
...
#   its own basic ambient correction. Skip directly to Step 6 (Doublet
#   Detection).
...
#     containing ALL barcodes (saved in Step 4) used to establish the soup
#     profile
...
# ****************************************************************************#
#   SoupX needs an approximate understanding of cell identity to figure out
#   which genes do not biologically belong in specific cells. For example,
#   if an entire cluster of T-cells expresses 3% Hemoglobin, SoupX knows
#   that is biological nonsense and tags Hemoglobin as ambient contamination.
#   We utilize standard Seurat pipelines linked via dplyr piping syntax
#   (`%>%`) to rapidly cluster the cells and feed the assignments directly
#   into SoupX.
...
#   Temporary object for clustering
...
#   a rapid clustering reference for SoupX. By evaluating the global
#   contamination fraction, our data-driven decision loop determined that our
#   sample is completely clean (returning a fraction of NULL/<5%). Consequently,
#   we safely skipped background subtraction to avoid introducing processing
#   artifacts, successfully preserving our pristine, unmanipulated raw
#   expression counts.
#
