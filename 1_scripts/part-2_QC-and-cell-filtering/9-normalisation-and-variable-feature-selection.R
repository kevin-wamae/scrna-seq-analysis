...
# ****************************************************************************#
#   Think of raw cell data like photos taken under different lighting. Some
#   cells look "brighter" (deep sequencing depth) purely due to technical luck.
#   If we don't normalize, the computer groups cells by how bright the photo is
#   instead of what is actually in it (the true biology). This step levels the
#   playing field.
#
...
#   defining biological variance is highly concentrated in the top 1,000 to
#   2,000 genes. Beyond 2,000, the variance line drops flat onto a baseline
#   shelf, meaning any additional gene rows you feed into the computer add a
#   heavy computational burden without contributing any useful biological
#   signal to your final UMAP or cluster maps.
...
#   Why we isolate genes: Out of ~20,000 genes, the vast majority are either
#   flat baseline background or household genes expressed equally across all
#   cell types (like actin). Keeping all of them adds massive computational
#   dead-weight.
#
#   Under the hood ("vst"):
#   - The Variance Stabilizing Transformation (VST) calculates a mean-
#     variance
...
ggsave(
  file.path(PLOTS_OUT_DIR, "07_variable_features.png"),
...
#   matrix. The matrix was clean of structural cell damage (Step 7) and
#   stripped of uninformative low-frequency gene features (Step 8). However,
#   the remaining raw expressions were still heavily distorted by uneven,
#   technical library sequencing sizes across individual droplets.
#
# WHAT WE HAVE ACCOMPLISHED:
#   In this step, we successfully completed our foundational data conversion
#   milestone by running Log-Normalization and Feature Selection. By scaling
#   cell transcript totals to a standard 10,000 factor and applying a log1p
#   shift, we removed technical depth artifacts, making cellular volumes
#   mathematically comparable. We then used a Variance Stabilizing Transfor-
#   mation (vst) to cut through baseline background noise and plot our variable
#   features chart (07_variable_features.png). This process accurately isolated
#   the top 2,000 highly variable gene drivers (such as immunoglobulins like
#   IGKC and IGLC2) that contain real, high-variance biological cell-type
#   signals.
#
...
#   tracking 2,000 separate gene axes is too mathematically complex for a
#   computer to cluster cleanly, our next phase will use Principal Component
#   Analysis (PCA) to compress these 2,000 variable dimensions down into 30
#   core principal components. This structural compression prepares our clean
#   sample for true graph-based cell clustering and native 2D UMAP
#   visualization.
# ****************************************************************************#
